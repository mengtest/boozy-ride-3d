...
public class WooglieEditor : EditorWindow
{
    ...
    void SetupData()
    {
        ...
        //PRO NO LONGERE REQUIRED IN Unity 5
    ...
    void DownloadEditMetaData()
    {
        ...
        else
        {
            ...
            if (items.Length > 9) editGameInfo.gameTags = items[9];

            if (editGameInfo.title == "" || editGameInfo.title == "-")
            {
                //New items
                editGameInfo.gameWidth = PlayerSettings.defaultWebScreenWidth;
                editGameInfo.gameHeight = PlayerSettings.defaultWebScreenHeight;
            }
    ...
    public static string TMPFolder()
    {
        ...
        string pat = Application.dataPath.Replace("/Assets", "/") + "Builds/";
        EnsureFolders(pat);
        return pat;
    }
    ...
    void Publish()
    {
        ...
        WooglieAPI obj2 = (WooglieAPI)GameObject.FindObjectOfType(typeof(WooglieAPI));
        if (obj2 != null)
        {
    ...
    string StartBuild()
    {
        ...
        {
            PlayerSettings.defaultWebScreenWidth = editGameInfo.gameWidth;
            PlayerSettings.defaultWebScreenHeight = editGameInfo.gameHeight;
        }
        ...
        string errorR = BuildPipeline.BuildPlayer(GetBuildScenes(), outputFolder + "Wooglie", target, BuildOptions.None);
    ...
    void UploadWebplayer()
    {
        Dictionary<string, WWW> localFiles = new Dictionary<string, WWW>();
        foreach (string str in Directory.GetFiles(TMPFolder() + "/Wooglie/Compressed"))
        {
            FileInfo fi = new FileInfo(str);

            if (fi.Extension == ".gz" || fi.Extension == ".datagz" || fi.Extension == ".memgz" || fi.Extension == ".jsgz")
            {
                string filename = ("file://" + fi.FullName).Replace("\\", "/");
                localFiles.Add(fi.Name, new WWW(filename));
            }
            else
            {
                Debug.LogError("Could not recognize file extension: " + fi.Extension + " [" + fi.FullName + "] new WebGL file?");
            }
        }

        int mb = 0;
        foreach (WWW w in localFiles.Values)
        {
            mb += w.bytes.Length / (1024 * 1024);
        }
        Debug.Log(mb + " MBS");

        if (mb > 25)
        {
            EditorUtility.DisplayDialog("Error", "The game is " + mb + "Mb. THe max. (compressed) upload size is 25MB. Please optimize your game. ", "OK");
            return;
        }
        else if (mb < 1)
        {
            EditorUtility.DisplayDialog("Error", "The API tried to upload a file with filesize <=1mb (" + mb + "mb). Please contact hello@Wooglie.com so that we can help you upload your game.", "OK");
            return;
        }
        ...
        form.AddField("buildtype", "webgl");

        int fileNr = 0;
        foreach (KeyValuePair<string, WWW> kvp in localFiles)
        {
            WWW w = kvp.Value;
            form.AddBinaryData("newUnityFile" + fileNr, w.bytes, kvp.Key);
            fileNr += 1;
        }
    ...
    string Contact(WWWForm form)
    {
        ...
        double timeout = -1;
        while (!www.isDone && (timeout == -1 || timeout > EditorApplication.timeSinceStartup))
        {
            /*if (www.uploadProgress >= 1 && timeout==-1)
            {
                timeout = EditorApplication.timeSinceStartup + 300;
            }*/
            //HANG
